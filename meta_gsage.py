...
parser.add_argument(
    ...
    type=int,
    default=8,
    help="Number of hidden units in the GraphSAGE target model.",
)
parser.add_argument(
...
parser.add_argument(
    ...
    type=float,
    default=0.01,
    help="Learning rate for retraining the GraphSAGE target model.",
)
parser.add_argument(
    "--target_patience",
    type=int,
    default=30,
    help="Early-stopping patience for the GraphSAGE target model.",
)
parser.add_argument(
    "--dataset",
...
parser.add_argument(
    ...
    type=int,
    default=0,
    help="Number of retries for text generation quality checks and transient LLM API errors (0=no retry).",
)
parser.add_argument(
...
parser.add_argument(
    ...
    type=str,
    default="local_ae_ppr",
    choices=["random", "local_degree", "global_pagerank", "local_ae_ppr"],
    help="Attribute candidate selection strategy after each structural endpoint is found.",
)
parser.add_argument(
    "--local_candidate_hops",
    type=int,
    default=2,
    help="Hop radius for random and local-degree attribute candidate pools around the structural endpoints.",
)
parser.add_argument(
...
log_filename = f"logs/result_{args.dataset}_{args.model}_GSAGE_{timestamp}.txt"
...
def test(adj, features, idx_eval, description="Test set"):
    """Retrain and evaluate a GraphSAGE target model on the supplied graph."""
    result = train_and_evaluate_target(
        target_model_name="gsage",
        adj=adj,
        features=features,
        labels=labels,
        idx_train=idx_train,
    ...
    print(
        f"{description} GraphSAGE results:",
...
def main():
    ...
    print("  Surrogate / target: GCN / GraphSAGE")
    print(
        "  Structure search: "
    ...
    print("\n" + "=" * 60)
    print("📊 GraphSAGE Transfer Evaluation Results")
    print("=" * 60 + "\n")
